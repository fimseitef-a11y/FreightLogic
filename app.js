...
/** Freight Logic v13.2.0-omega-midwest
 *  All P0-P4 + Profit Engine (Load Decision Score)
 */

const APP_VERSION = '13.2.0-omega-midwest';
...
function closeModal(){
  ...
  setTimeout(()=>{ bd.style.display = 'none'; md.style.display = 'none'; $('#modalBody').innerHTML = ''; }, 350);
}
...
function sanitizeTrip(raw){
  ...
  t.pay = Number(raw.pay || 0);
  t.loadedMiles = Math.max(0, Number(raw.loadedMiles || 0));
  t.emptyMiles = Math.max(0, Number(raw.emptyMiles || 0));
  ...
  t.created = Number(raw.created || Date.now());
  t.updated = Date.now();
  return t;
}
...
async function upsertTrip(trip){
  ...
  stores.trips.put(t);
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: t.orderNo, action:'UPSERT_TRIP', data: t });
...
async function deleteTrip(orderNo){
  ...
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: orderNo, action:'DELETE_TRIP' });
...
function sanitizeExpense(raw){
  return { id: raw.id ? Number(raw.id) : undefined, date: raw.date || isoDate(),
    amount: Math.max(0, Number(raw.amount || 0)), category: clampStr(raw.category, 60),
    notes: clampStr(raw.notes, 300), created: Number(raw.created || Date.now()),
...
async function addExpense(exp){
  ...
  return new Promise((resolve,reject)=>{
    ...
    req.onsuccess = ()=>{
      e.id = req.result;
      try{ stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(e.id), action:'ADD_EXPENSE', data: e }); }catch{}
    };
...
async function updateExpense(exp){
  ...
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(e.id), action:'UPDATE_EXPENSE', data: e });
...
async function deleteExpense(id){
  ...
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(id), action:'DELETE_EXPENSE' });
  return new Promise((resolve,reject)=>{ txn.oncomplete = ()=> resolve(true); txn.onerror = ()=> reject(txn.error); });
}
...
function sanitizeFuel(raw){
  return { id: raw.id ? Number(raw.id) : undefined, date: raw.date || isoDate(),
    gallons: Math.max(0, Number(raw.gallons || 0)), amount: Math.max(0, Number(raw.amount || 0)),
    state: clampStr(raw.state, 20), notes: clampStr(raw.notes, 200),
    created: Number(raw.created || Date.now()), updated: Date.now() };
}
async function addFuel(f){
  ...
  return new Promise((resolve,reject)=>{
    req.onsuccess = ()=> { x.id = req.result; try{ stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(x.id), action:'ADD_FUEL', data: x }); }catch{} };
    req.onerror = ()=> reject(req.error);
...
async function updateFuel(f){
  ...
  const {t:txn, stores} = tx(['fuel','auditLog'],'readwrite');
  stores.fuel.put(x);
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(x.id), action:'UPDATE_FUEL', data: x });
  return new Promise((resolve,reject)=>{ txn.oncomplete = ()=> resolve(x); txn.onerror = ()=> reject(txn.error); });
}
async function deleteFuel(id){
  const {t:txn, stores} = tx(['fuel','auditLog'],'readwrite');
  stores.fuel.delete(Number(id));
  stores.auditLog?.put?.({ id: crypto.randomUUID?.() || String(Date.now())+Math.random(), timestamp: Date.now(), entityId: String(id), action:'DELETE_FUEL' });
  return new Promise((resolve,reject)=>{ txn.oncomplete = ()=> resolve(true); txn.onerror = ()=> reject(txn.error); });
}
...
async function exportTripsCSV(){
  const trips = await dumpStore('trips');
  const header = ['Order#','Customer','Pickup','Delivery','Origin','Destination','Pay','LoadedMiles','EmptyMiles','AllMiles','RPM','Paid','PaidDate','Notes'];
  const rows = [header, ...trips.map(t => {
    ...
    return [t.orderNo, t.customer, t.pickupDate, t.deliveryDate, t.origin, t.destination, t.pay, t.loadedMiles, t.emptyMiles, all, rpm, t.isPaid?'Yes':'No', t.paidDate||'', t.notes];
  })];
...
async function importJSON(file){
  try{
    ...
    const ALLOWED_SETTINGS_KEYS = new Set(['uiMode','perDiemRate','brokerWindow','weeklyGoal','iftaMode','omegaLastInputs','lastExportDate','vehicleMpg','fuelPrice','weeklyReflection']);
    const safeSettingsArr = arr(data.settings).filter(s => s && typeof s === 'object' && typeof s.key === 'string' && ALLOWED_SETTINGS_KEYS.has(s.key)).map(s => ({
      key: s.key, value: typeof s.value === 'object' && s.value !== null ? JSON.parse(JSON.stringify(s.value)) : s.value
    }));
    ...
    const safeAuditArr = arr(data.auditLog).filter(a => a && typeof a === 'object' && typeof a.id === 'string' && typeof a.timestamp === 'number' && typeof a.action === 'string').map(a => ({
      ...
      action: clampStr(a.action, 30), data: a.data && typeof a.data === 'object' ? JSON.parse(JSON.stringify(a.data)) : undefined
    }));
...
async function computeTaxView(trips, exps){
  ...
  const perDiem = perDiemRate > 0 ? (perDiemRate * days.size) : 0;
  const se = Math.max(0, (net - perDiem) * 0.9235 * 0.153); // IRS SE tax: deduct per diem first
  const profit = net - perDiem - se;
...
const views = { home:$('#view-home'), trips:$('#view-trips'), expenses:$('#view-expenses'),
  money:$('#view-money'), fuel:$('#view-fuel'), insights:$('#view-insights'), omega:$('#view-omega') };
    ...
    if (f.cached){
      viewBtn.addEventListener('click', async ()=>{
        try{ const data = await cacheGetReceipt(f.id); if (!data){ toast('Receipt not in cache', true); return; }
          const url = URL.createObjectURL(data.blob); window.open(url, '_blank'); setTimeout(()=> URL.revokeObjectURL(url), 30000);
...
function parseLoadText(text){
  ...
  const shipperMatch = full.match(/(?:shipper|pick\s*up|origin)\s*:?\s*(.{5,40}?)(?:\n|,\s*[A-Z]{2}|$)/i);
  const consigneeMatch = full.match(/(?:consignee|deliver(?:y)?|destination|drop)\s*:?\s*(.{5,40}?)(?:\n|,\s*[A-Z]{2}|$)/i);
...
function pickFile(accept){
  return new Promise((resolve)=>{ const i = document.createElement('input'); i.type = 'file'; i.accept = accept; i.onchange = ()=> resolve(i.files?.[0] || null); i.click(); });
}
$('#btnTripImport').addEventListener('click', async ()=>{
  const f = await pickFile('application/json'); if (!f) return;
  await importJSON(f); invalidateKPICache(); await renderTrips(true); await renderHome();
});
$('#btnExpImport').addEventListener('click', async ()=>{
  const f = await pickFile('application/json'); if (!f) return;
  await importJSON(f); invalidateKPICache(); await renderExpenses(true); await renderHome();
});
...
async function generateAccountantPackage(period='ytd'){
  ...
  try{
    ...
    const seTax = Math.max(0, (netIncome - perDiemTotal) * seRate * 0.9235); // 92.35% of net
    const estimatedProfit = netIncome - perDiemTotal - seTax;
    ...
    const summaryRows = [
      ...
      ['Per Diem Total', '$' + perDiemTotal.toFixed(2)],
      [''],
      ['BOTTOM LINE'],
      ['Net Income (Revenue - Expenses)', '$' + netIncome.toFixed(2)],
      ['Per Diem Deduction', '-$' + perDiemTotal.toFixed(2)],
