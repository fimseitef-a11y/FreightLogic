...
const APP_VERSION = '14.3.4-hardened';
...
function sanitizeImportValue(val){
  ...
  s = s.replace(/^[\t\r\n]+/, '');
  while (/^[=+\-@|%!]/.test(s) && s.length > 1) s = s.slice(1);
...
async function mwInit(){
  ...
  const tabBtn = document.querySelector(`#mwTabs [data-mwtab="${lastTab}"]`);
...
async function loadTesseract(){
  ...
  if (typeof Tesseract === 'undefined'){
    await new Promise((resolve, reject) => {
      const s = document.createElement('script');
      // SECURITY: Version-pinned (not @5 range). Generate SRI hash via:
