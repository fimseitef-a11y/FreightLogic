  ...
  try {
    ...
    for (const reg of regs) {
      if (reg.active) {
        ...
        const isStale = ver === '__TIMEOUT__' || !ver || !ver.includes('10.3.0');
        if (isStale) {
...
function assertCriticalUI() {
  ...
  navTabs.forEach(btn => {
    ...
    if (!tabId) return;
    const panel = document.getElementById('m-' + tabId);
    if (!panel) throw new Error(`[UI] Missing tab panel for data-tab="${tabId}" (expected id="m-${tabId}")`);
  });
}
...
// ─── PERF: Coalesced refresh — batches multiple refreshUI() into one rAF ───
...
function scheduleRefresh() {
  ...
  _refreshRAF = requestAnimationFrame(async () => {
    _refreshPending = false;
    try { await refreshUI(); } catch (e) { console.error('[scheduleRefresh]', e); }
  });
}
  ...
  window.addEventListener('load', () => {
    ...
    navigator.serviceWorker.register('service-worker.js', { scope: './' })
      .then(reg => {
        ...
        // PERF: hourly update check handled by UpdateModule.init() — no duplicate interval
      })
...
function initOnboarding() {
  const flag = localStorage.getItem('xp_onboarded') || localStorage.getItem('fl_onboarded');
  if (flag === 'true') return;
  ...
  function showStep(n) {
    ...
    document.querySelectorAll('.onboard-dot').forEach(d => d.style.background = 'var(--border)');
    const stepEl = document.querySelector(`.onboard-step[data-step="${n}"]`);
    const dotEl = document.querySelector(`.onboard-dot[data-dot="${n}"]`);
    if (stepEl) stepEl.style.display = '';
    if (dotEl) dotEl.style.background = 'var(--accent-primary)';
    const nextBtn = $('btnOnboardNext');
    if (nextBtn) nextBtn.textContent = n >= maxSteps ? 'Get Started ✓' : 'Next →';
  }

  function closeOnboard() {
    welcome.classList.remove('active');
    localStorage.setItem('xp_onboarded', 'true');
    localStorage.setItem('fl_onboarded', 'true');
  }

  $safe('btnOnboardNext').addEventListener('click', () => {
    if (step >= maxSteps) { closeOnboard(); return; }
    step++;
    showStep(step);
  });
  $safe('btnOnboardSkip').addEventListener('click', closeOnboard);
  $safe('closeWelcomeModal').addEventListener('click', closeOnboard);
}
...
async function exportAccountantPacket() {
  try {
    ...
    let fuel      = (await dbOp('fuel','readonly', s => s.getAll())).filter(isActiveRecord);
    let expenses  = (await dbOp('expenses','readonly', s => s.getAll())).filter(isActiveRecord);
...
async function beginGPSTracking() {
  ...
  $('activeTripID').textContent = gpsTrackingData.orderNo;
...
async function saveTrip() {
  ...
  try {
    ...
    if (typeof _flIncrementSaveCount === 'function') _flIncrementSaveCount();
    await checkStorageQuota();
    scheduleRefresh();
  } catch (e) {
...
function clearTripForm() {
  ...
  try { localStorage.removeItem('fl_tripDraft'); } catch(e) {}
  // Reset edit-mode UI
  const formSection = $('tripFormSection');
  if (formSection) { formSection.style.borderColor = ''; }
  const title = $('tripFormTitle');
  if (title) title.textContent = 'New Trip';
}

// UX: Tap-to-edit — pre-populate form with existing trip data
async function openEditTrip(orderNo) {
  try {
    const trip = await dbOp('trips', 'readonly', s => s.get(orderNo));
    if (!trip) return showToast('Trip not found', true);
    
    // Switch to trips tab if not already there
    switchTab('trips');
    
    // Pre-populate form
    $('orderNo').value = trip.orderNo || '';
    $('customer').value = trip.customer || '';
    $('origin').value = trip.origin || '';
    $('dest').value = trip.dest || '';
    $('loadedMiles').value = trip.loadedMiles || '';
    $('emptyMiles').value = trip.emptyMiles || '';
    $('revenue').value = trip.revenue || '';
    $('pickupDate').value = trip.pickupDate || '';
    $('deliveryDate').value = trip.deliveryDate || '';
    $('tripNotes').value = trip.notes || '';
    $('tripPaid').checked = !!trip.paid;
    
    // Expand details section (has customer/route/etc)
    const details = $('tripDetailsSection');
    const detailsBtn = $('btnToggleTripDetails');
    if (details) details.style.display = '';
    if (detailsBtn) detailsBtn.textContent = '▴ Hide Details';
    
    // Visual indicator: editing mode
    const formSection = $('tripFormSection');
    if (formSection) formSection.style.borderColor = 'var(--accent-info)';
    const title = $('tripFormTitle');
    if (title) title.textContent = '✏️ Edit: ' + orderNo;
    
    // Scroll form into view
    setTimeout(() => {
      $('orderNo').scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 100);
    
    showToast('📝 Editing — modify fields and save');
  } catch (e) {
    showToast('Error loading trip', true);
  }
}
...
async function saveFuel() {
  ...
  try {
    ...
    showToast('⛽ Fuel saved'); if (typeof _flIncrementSaveCount === 'function') _flIncrementSaveCount(); checkStorageQuota(); scheduleRefresh();
...
async function saveExpense() {
  ...
  try {
    ...
    showToast(isRecurring ? '💰 Recurring expense saved' : '💰 Expense saved'); if (typeof _flIncrementSaveCount === 'function') _flIncrementSaveCount(); checkStorageQuota(); scheduleRefresh();
...
async function saveMeal() {
  ...
  try {
    ...
    showToast('🍔 Meal saved'); if (typeof _flIncrementSaveCount === 'function') _flIncrementSaveCount(); checkStorageQuota(); scheduleRefresh();
...
function renderTripCards(trips) {
  ...
  trips.forEach(t => {
    ...
    editBtn.addEventListener('click', (e) => { e.stopPropagation(); openEditTrip(clean.orderNo); });
    actions.appendChild(editBtn);
    
    const delBtn = document.createElement('button');
    delBtn.className = 'card-action-btn delete';
    delBtn.textContent = '🗑️ Delete';
    delBtn.addEventListener('click', (e) => { e.stopPropagation(); confirmDeleteRecord('trips', clean.orderNo, 'trip'); });
...
async function refreshCommandCenter(preTrips, preFuel, preExpenses) {
  try {
    const trips = preTrips || (await dbOp('trips', 'readonly', s => s.getAll())).filter(isActiveRecord);
    const fuel = preFuel || (await dbOp('fuel', 'readonly', s => s.getAll())).filter(isActiveRecord);
    const expenses = preExpenses || (await dbOp('expenses', 'readonly', s => s.getAll())).filter(isActiveRecord);
...
async function refreshARPanel() {
  ...
  try {
    ...
    renderARPanel(data);
    // v10.2.1: Refresh customer rate history when AR tab loads
...
async function refreshSummary() {
  try {
    ...
    const qs = (id) => $(id);
    if (qs('qsRevenue')) qs('qsRevenue').textContent = formatMoney(a.grossRevenue);
    if (qs('qsExpenses')) qs('qsExpenses').textContent = formatMoney(a.totalDeductions);
    if (qs('qsTax')) qs('qsTax').textContent = a.isLoss ? '$0' : formatMoney(a.totalTax);
    if (qs('qsNet')) {
      const netAfterTax = a.isLoss ? a.grossProfit : a.grossProfit - a.totalTax;
      qs('qsNet').textContent = formatMoney(netAfterTax);
      qs('qsNet').className = 'stat-val ' + (netAfterTax >= 0 ? 'text-success' : 'text-danger');
    }
...
function exportCSV() {
  ...
  ]).then(([rawTrips, rawFuel, rawExpenses]) => {
    const trips = rawTrips.filter(isActiveRecord);
    const fuel = rawFuel.filter(isActiveRecord);
    const expenses = rawExpenses.filter(isActiveRecord);
...
function importUnifiedCSV(file) {
  ...
  reader.onload = async e => {
    try {
      ...
      scheduleRefresh(); checkStorageQuota();
      $('fileImportCSV').value = '';
    } catch (e) {
...
async function saveHomeAddress() {
  ...
  try {
    ...
    updateTaxInfoDisplay(); scheduleRefresh();
...
async function refreshDashboard() {
  try {
    ...
    // Freight Score
    const fsEl = $('dashFreightScore');
    const fsSub = $('dashFreightScoreSub');
    if (fsEl && typeof calculateFreightScore === 'function') {
      const totExp = allFuel.reduce((s,f) => s + safeFloat(f.amount), 0)
                   + allExpenses.reduce((s,e) => s + safeFloat(e.amount), 0)
                   + allMeals.reduce((s,m) => s + safeFloat(m.amount), 0);
      const fs = calculateFreightScore({
        totalRevenue: totRev, totalExpenses: totExp,
        totalMiles: allTrips.reduce((s,t) => s + safeFloat(t.loadedMiles) + safeFloat(t.emptyMiles), 0),
        unpaidRevenue: unpaidTotal, netProfit: totRev - totExp, trips: allTrips
      });
      fsEl.textContent = fs.score || '—';
      if (fs.color) fsEl.style.color = fs.color;
      if (fsSub) fsSub.textContent = fs.detail || '';
    }
...
console.log('[App] Freight Logic v10.2.1 initializing');
  ...
  // Integrations (deep links) — removed from form, kept functions for trip card context menus
  $safe('origin').addEventListener('input', () => {});
  $safe('dest').addEventListener('input', () => {});
  ...
  // Pagination
  $safe('btnPrevPage').addEventListener('click', () => { currentPage--; scheduleRefresh(); });
  $safe('btnNextPage').addEventListener('click', () => { currentPage++; scheduleRefresh(); });
  ...
  // PERF: Event-driven quota check instead of 60s polling (saves battery)
  // Checks run: on startup (above), after saves, and when app returns to foreground
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') checkStorageQuota();
  });

  scheduleRefresh();

  // Event Listeners (continued)
  // simBtn handled in v10.2.1 features block above
  ...
  if (urlParams.get('share-target') === 'received') {
    try {
      const cache = await caches.open('freight-logic-shared-files-v10.2.1');
  ...
  console.log('[App] Freight Logic v10.2.1 Ready');
}).catch(e => {
...
// ============================================================================
// v10.3.0 FEATURES (outside IIFE — uses global $ and helpers)
// ============================================================================

// ── FREIGHT SCORE (Composite Financial Health 0-100) ──────────────────────
function calculateFreightScore(data) {
  if (!data || !data.trips || data.trips.length === 0) 
    return { score: 0, label: '—', color: 'var(--text-tertiary)', detail: 'Enter trips to see score' };
  
  const { totalRevenue, totalExpenses, unpaidRevenue, netProfit } = data;
  let score = 50;
  ...
  else if (margin > 0.08) score += 12;
  else if (margin > 0) score += 5;
  else score -= 15;
  
  // A/R health (±15 pts)
  const arRatio = totalRevenue > 0 ? ((unpaidRevenue || 0) / totalRevenue) : 0;
  if (arRatio < 0.1) score += 15;
  else if (arRatio < 0.25) score += 10;
  else if (arRatio < 0.5) score += 5;
  else score -= 5;
  
  // Expense ratio (±10 pts)
  const expRatio = totalRevenue > 0 ? (totalExpenses / totalRevenue) : 1;
  if (expRatio < 0.65) score += 10;
  else if (expRatio < 0.80) score += 5;
  else score -= 5;
  
  score = Math.max(0, Math.min(100, Math.round(score)));
  
  let label, color;
  if (score >= 80) { label = 'Excellent'; color = 'var(--accent-success)'; }
  else if (score >= 60) { label = 'Good'; color = 'var(--accent-primary)'; }
  else if (score >= 40) { label = 'Fair'; color = 'var(--accent-warning)'; }
  else { label = 'At Risk'; color = 'var(--accent-danger)'; }
  
  return { score, label, color, detail: `${label} · ${Math.round(margin*100)}% margin` };
}
...
(function() {
  ...
  const chevron = $('dashChevron');
  if (!toggle || !content) return;
  
  if (localStorage.getItem('fl_dashCollapsed') === 'true') {
  ...
  toggle.addEventListener('click', () => {
    ...
    localStorage.setItem('fl_dashCollapsed', !isHidden);
  });
})();
...
(function() {
  const $ = id => document.getElementById(id);
  const btn = $('btnToggleTripDetails');
  const section = $('tripDetailsSection');
  if (!btn || !section) return;
  
  if (localStorage.getItem('fl_tripDetailsOpen') === 'true') {
  ...
  btn.addEventListener('click', () => {
    ...
    btn.textContent = isHidden ? '▴ Hide Details' : '▾ More Details (customer, route, dates, notes)';
    localStorage.setItem('fl_tripDetailsOpen', isHidden);
  });
})();

// ── TAX PROGRESSIVE DISCLOSURE ────────────────────────────────────────────
(function() {
  const $ = id => document.getElementById(id);
  const btn = $('btnTaxDetails');
  const section = $('taxDetailsSection');
  if (!btn || !section) return;
  
  btn.addEventListener('click', () => {
    const isHidden = section.style.display === 'none';
    section.style.display = isHidden ? '' : 'none';
    btn.textContent = isHidden ? '▴ Hide Full Tax Breakdown' : '▾ Show Full Tax Breakdown';
  });
})();
...
(function() {
  const $ = id => document.getElementById(id);
  const DRAFT_KEY = 'fl_tripDraft';
  const fields = ['orderNo','customer','origin','dest','loadedMiles','emptyMiles','revenue','pickupDate','deliveryDate','tripNotes'];
  ...
  function saveDraft() {
    ...
    saveTimer = setTimeout(() => {
      ...
      const paid = $('tripPaid');
      if (paid && paid.checked) { draft.tripPaid = true; hasData = true; }
  ...
  try {
    ...
    if (raw) {
      ...
      if (draft.tripPaid) { const p = $('tripPaid'); if (p) p.checked = true; }
      if (restored) {
        // If detail fields have data, expand section
        if (draft.customer || draft.origin || draft.dest || draft.pickupDate) {
          const section = $('tripDetailsSection');
          const btn = $('btnToggleTripDetails');
  ...
  const paid = $('tripPaid');
  if (paid) paid.addEventListener('change', saveDraft);
})();

// ── BACKUP STATUS BADGE + SAVE COUNTER ────────────────────────────────────
function _flIncrementSaveCount() {
  try {
    let count = parseInt(localStorage.getItem('fl_saveCount') || '0', 10) + 1;
    localStorage.setItem('fl_saveCount', String(count));
    _flUpdateBackupBadge();
  } catch(e) {}
}

function _flUpdateBackupBadge() {
  const badge = document.getElementById('lastBackupBadge');
  if (!badge) return;
  const last = localStorage.getItem('backupLastExport');
  if (last) {
    const daysSince = Math.floor((Date.now() - parseInt(last, 10)) / 86400000);
    if (daysSince === 0) badge.textContent = '💾 Today';
    else if (daysSince === 1) badge.textContent = '💾 Yesterday';
    else badge.textContent = '💾 ' + daysSince + 'd ago';
    badge.style.color = daysSince > 7 ? 'var(--accent-warning)' : 'var(--accent-success)';
    badge.title = 'Last backup: ' + daysSince + ' days ago';
  } else {
    badge.textContent = '⚠️ No backup';
    badge.style.color = 'var(--accent-warning)';
    badge.title = 'No backup yet — data only on this device';
  }
}
_flUpdateBackupBadge();
...
(function() {
  ...
  bind('btnReportExportBackup', () => { if (typeof exportEncryptedBackup === 'function') exportEncryptedBackup(); });
  bind('btnReportExportAccountant', () => { if (typeof exportAccountantPacket === 'function') exportAccountantPacket(); });
  bind('btnReportExportCSV', () => { if (typeof exportCSV === 'function') exportCSV(); });
})();