...
const SHARED_CACHE = 'freight-logic-shared-files-v10.2.1';
...
self.addEventListener('install', event => {
  console.log('[SW] v10.2.1 installing...');
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .then(() => {
        console.log('[SW] v10.2.1 installed');
...
self.addEventListener('activate', event => {
  console.log('[SW] v10.2.1 activating...');
  event.waitUntil((async () => {
    try {
      ...
      console.log('[SW] v10.2.1 activated');

      // Notify all clients that new version is active
      const clients = await self.clients.matchAll();
      clients.forEach(client => {
        client.postMessage({ type: 'SW_UPDATED', version: 'v10.2.1' });
      });
...
console.log('[SW] v10.2.1 loaded and ready');
