// Freight Logic Service Worker v10.2.0 — Enterprise Hardened — Enterprise Hardened
// NOTE: Security posture preserved (no "cache every GET" regression).

const CACHE_NAME = 'freight-logic-v10.2.0';
const RUNTIME_CACHE = 'freight-logic-runtime-v10.2.0';
const MAX_RUNTIME_ENTRIES = 60;
const RUNTIME_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 days

const SHARED_CACHE = 'freight-logic-shared-files-v10.1.1';
...
self.addEventListener('install', event => {
  console.log('[SW] v10.1.1 installing...');
  event.waitUntil(
    caches.open(CACHE_NAME)
      ...
      .then(() => {
        console.log('[SW] v10.1.1 installed');
...
self.addEventListener('activate', event => {
  console.log('[SW] v10.1.1 activating...');
  event.waitUntil((async () => {
    try {
      ...
      console.log('[SW] v10.1.1 activated');

      // Notify all clients that new version is active
      const clients = await self.clients.matchAll();
      clients.forEach(client => {
        client.postMessage({ type: 'SW_UPDATED', version: 'v10.1.1' });
      });
...
console.log('[SW] v10.1.1 loaded and ready');
